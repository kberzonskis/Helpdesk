...
export function EditStoryForm({ api, method, story }) 
{   const imgPath = story.img ? (SERVER_ADDRESS + '/img/stories/' + story.img) : defaultImg;
    const [title, setTitle] = useState(story?.title ?? '');
    const [url, setUrl] = useState(story?.url_slug ?? '');
    const [description, setDescription] = useState(story?.description ?? '');
    const [status, setStatus] = useState(story?.status_name ?? 'draft');
    const [img, setImg] = useState(story?.img ?? '');
    const [imgErr, setImgErr] = useState('');

    const { updateAdminStories, updatePublicStories } = useContext(StoriesContext);
               ...
               <form onChange={handleImageFormSubmit} className="col-12 col-md-9 col-lg-6 mt-5">
                <img id="img_preview" className="d-block w-100 object-fit-contain"
                    style={{ height: '20rem', backgroundColor: '#eee' }}
                     ...
                     src={img ? (imgPath):(SERVER_ADDRESS + img)   }alt="Story thumbnail" />
                     <p id="img_path">{img}</p>
